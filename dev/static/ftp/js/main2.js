...
const submitProjectForm = (e) => {
    ...
    const ftpSOP = formData.get("ftpSOP");
    const fileCV = formData.get("fileCV");

    if (e.target.classList.contains("projectForm__disabledBtn"))
        openWordCountAlert(e);
    else {
        if (!confirm('Are you sure you want to submit? You cannot change the details later.')) { return false; }

        $.ajax({
            type: "POST",
            url: "{% url 'ftp:apply_project' project_id=p.id %}",
            data: {
                'pid': pid,
            },
            dataType: 'json',
